...
@section('content')
        ...
        <form method="POST" action="{{ route('penelitian-pkm.update', $penelitianPkm) }}">
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium mb-1">
                    Dosen Anggota
                    <span style="color: var(--muted-foreground);">(opsional)</span>
                </label>
                ...
                {{-- Dosen yang sudah dipilih --}}
                <div id="selected-dosen" class="mt-3 flex flex-wrap gap-2"></div>

                {{-- ID dosen yang dikirim ke controller --}}
    ...
    // Dosen yang sudah menjadi anggota
    let selectedDosen = [
        @foreach ($penelitianPkm->dosen->where('pivot.peran', 'Anggota') as $dosen)
            {
                id: {{ $dosen->id }},
                nama: @json($dosen->nama)
            },
        @endforeach
    ];
    ...
    function removeDosen(id) {
        selectedDosen = selectedDosen.filter(
            dosen => dosen.id !== id
        );
    ...
    searchInput.addEventListener('input', function () {
        ...
        results.forEach(dosen => {
            ...
            item.type = 'button';
            item.className =
                'w-full text-left px-4 py-3 text-sm hover:opacity-70';
    ...
    // Tampilkan anggota yang sudah tersimpan
    renderSelectedDosen();
</script>
@endsection